...
import com.martdev.android.ministrydiary.utils.ADD_EDIT_RESULT_OK
...
class BibleStudentsViewModel(private val bibleStudentRepo: BibleStudentRepo) : ViewModel() {
    ...
    get() {
        Calendar.getInstance().apply {
            return GregorianCalendar(get(Calendar.YEAR), get(Calendar.MONTH), get(Calendar.DAY_OF_MONTH)).time
        }
    }
    ...
    fun loadData() {
        ...
        viewModelScope.launch {
            ...
            if (bibleStudentResult is Success) {
                ...
                if (bibleStudents != null) {
                    if (bibleStudents.isEmpty()) {
    ...
    fun filterByDateOfVisit() {
        ...
        viewModelScope.launch {
            ...
            if (bibleStudentResult is Success) {
                ...
                if (filteredBibleStudents != null) {
                    if (filteredBibleStudents.isEmpty()) {
    ...
    fun checkResultMessage(result: Int) {
        when(result) {
            ADD_EDIT_RESULT_OK -> showSnackbarMessage(R.string.added_bible_student)
